...
def myratings(request):
    ...
    return render_to_response('bmyratings.html', c,
...
def feed_rec(request):
  ...
  c = {'user':request.user,
        'formset':formset}
  c.update(csrf(request))
  return render_to_response('bfeedrec.html', c, context_instance=RequestContext(request))

@login_required(login_url='/login/')
def get_rec(request):
    from  book.webservice import WebService
    w = WebService()
    ...
    form.recs = reclist
    return render(request, 'brecommendations.html', {
